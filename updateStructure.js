...
const workSheet = {
  ...
  productCategories :
  workBook.productCategories.Sheets["product_categories"],
...
for (let i = 1; i < data.userPayment.length; i++) {
  ...
  if(data.userPayment[i][1] && data.userPayment[i][4] == true){
    ...
    userPaymentQuery.push([
      ...
      }', '${data.userPayment[i][3]}', '${
        "settlement"
      }',${data.userPayment[i][4]}, ${data.userPayment[i][5]}, ${
    ...
    for(let j = 1;j < data.user.length;j++){
      ...
      if(userId == data.user[j][0]){
        for(let p = 1;p < data.midtrans.length;p++){
          if(data.midtrans[p][8] == data.user[j][2] && data.midtrans[p][5] != "settlement" && data.midtrans[p][3] == "Payment"){
            isValid = true
            let isSame = false
            ...
            if(!isSame){
              userPaymentQuery.push([
                ...
                `INSERT INTO user_payments (id, order_id, variant_id, user_id, transaction_status, is_payed, is_qr_booked, gross_amount, payment_type, created_at, updated_at, bib_number) VALUES('${
                  ...
                  data.userPayment[i][2]
                }', '${data.userPayment[i][3]}', '${data.midtrans[p][5]}',${data.userPayment[i][4]}, ${data.userPayment[i][5]}, ${
                  data.midtrans[p][4]
                },'${data.midtrans[p][3]}', '2025-03-21', '2025-03-21', null);`,
              ]);
